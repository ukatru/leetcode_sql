...
Create table  Employees_1875 (employee_id int, name varchar(30), salary int)
Truncate table Employees_1875
insert into Employees_1875 (employee_id, name, salary) values ('2', 'Meir', '3000')
insert into Employees_1875 (employee_id, name, salary) values ('3', 'Michael', '3000')
insert into Employees_1875 (employee_id, name, salary) values ('7', 'Addilyn', '7400')
insert into Employees_1875 (employee_id, name, salary) values ('8', 'Juan', '6100')
insert into Employees_1875 (employee_id, name, salary) values ('9', 'Kannon', '7400')

select distinct employee_id,name,salary, dense_rank() over(order by salary)  team_id 
from (SELECT e.employee_id, e.name, e.salary
FROM Employees as e, Employees as e1
WHERE e.salary = e1.salary AND e.employee_id <> e1.employee_id) a
order by team_id ,
employee_id


with a as (select salary, row_number() over(order by salary) team_id  from Employees e 
group by salary
having count(distinct employee_id) > 1
           )
           
           select employee_id,name, a.salary,team_id 
           from a join Employees e on a.salary = e.salary
