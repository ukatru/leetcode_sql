...
select sum(apple_count) apple_count, sum(orange_count) orange_count from ( select box_id,c.chest_id,b.apple_count + 
COALESCE(c.apple_count,0) apple_count ,b.orange_count + COALESCE(c.orange_count,0) orange_count from boxes b
left join chests c
on b.chest_id = c.chest_id) a